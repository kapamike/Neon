...
import org.sat4j.moco.algorithm.StochasticAlgorithm;
...
public class Log {
     ...
     * @see #setSuppressAssignments(boolean)
     */
    public static void successful(MOCOProblem problem) {
        int successful_mutation = problem.getSuccessfulMutation();
        int total_mutation = problem.getTotalMutation();
        int successful_improvement = problem.getSuccessfulImprovement();
        int total_improvement = problem.getTotalImprovement();
        if (total_mutation != 0 || total_improvement != 0) {
            String s_str = "c Successful smart mutations = " + successful_mutation;
            s_str += "\nc Total smart mutations = " + total_mutation;
            s_str += "\nc Successful smart improvements = " + successful_improvement;
            s_str += "\nc Total smart improvements = " + total_improvement;
            write(0, s_str);
        }
    }
