...
public class Instance {
    ...
    private int countVars() {
        ...
        for (int i = 0; i < nObjs(); ++i) {
            for (int k = 0; k < getObj(i).nSubObj(); k++){
                ReadOnlyVecInt lits = getObj(i).getSubObjLits(k);
                for (int j = 0; j < lits.size(); ++j) {
                    int x = Math.abs(lits.get(j));
                    n = x > n ? x : n;
                }
     ...
     */
    public IVec<PBConstr> getConstrs() { return this.constrs; }

    /**
     * Retrieves an objective in the MOCO instance.
     * @return The objectives.
     */
    public IVec<Objective> getObjs() { return this.objs; }
    ...
    /**
     * Writes a given expression to a given object.
     * @param w The writer object.
    ...
    public Set<Integer> getLits() {
        Set<Integer> lits = null;
        ReadOnlyVecInt literals;
        for (int i = 0; i < this.nConstrs(); i++) {
            literals = this.getConstr(i).getLHS().getLits();
            for (int j = 0; j < literals.size(); j++) {
                lits.add(literals.get(j));
            }
        }
        for (int i = 0; i < this.nObjs(); i++) {
            for (int j = 0; j < this.getObj(i).nSubObj(); j++) {
                literals = this.getObj(i).getSubObjLits(j);
                for (int k = 0; j < literals.size(); k++) {
                    lits.add(literals.get(k));
                }
            }
        }
        return lits;
